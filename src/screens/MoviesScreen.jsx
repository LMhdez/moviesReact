...
const MoviesScreen = () => {
  ...
  const buildMoviesUrl = () => {
    const baseUrl = "https://moviesfunctionapp.azurewebsites.net/api/GetMovies";
    let url = baseUrl;
    ...
    if (sortBy) {
      url +=
        selectedGenres.length > 0 ? `&sortBy=${sortBy}` : `?sortBy=${sortBy}`;
    }
  ...
  return (
    <div className="flex flex-col min-h-screen">
      ...
      <main className="flex flex-grow">
        <aside className="w-1/4 p-4 bg-gray-100 rounded shadow sticky top-0 h-full">
